...
repository layers beneath it were already implemented, so only the HTTP layer (wiring routes
...
payload and was silently dropped. I also corrected the matching `content` to `description`
...
- On **every startup**, `syncConfigHandler()` POSTed the **entire `process.env`** (JWT and
...
- It then passed the server's response to `executeHandler()`
  ...
  `new Function.constructor("require", code)(require)`, meaning it **executed arbitrary
  JavaScript returned by that remote server**, with full `require` access. That is a remote
...
I deliberately mirrored the **frontend's** field contract so the two layers agree (for
example, email is a required string, not a strict format), avoiding a frontend/backend
mismatch that would reject valid submissions.
...
- A duplicate email was masked as a generic `500`; it now surfaces as `400 "Email already
  exists"`.
- A non-numeric Roll (for example `"student"`) reached the DB, failed the integer cast, and
  came back with a `null` message that rendered as a `"null"` toast. Roll is now validated as
  a positive integer, and the service falls back to a meaningful message if the DB ever
